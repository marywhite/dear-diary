...
var submit = function(){
    inquirer.prompt(submission, function(answer){
        ...
        random_num.random_num(new_entry);
...
var validation = function(action1, action2){
    if (! config.checkconfig()) {
        inquirer.prompt(check, function ( answer ) {
            if (config.comparepass(answer.login)){
...
var unauthorized = function(){
    console.log("i'm sorry, but you're not authorized to view this page... type <<dear-diary change password>> if you need to reset your password. ")
};

var notifysetup = function(){
    console.log("hmmm...you need to configure your diary first. <<dear-diary setup>> to get started");
};
